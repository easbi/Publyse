...
<div class="page-header row no-gutters py-4">
	<div class="col-12 col-sm-4 text-center text-sm-left mb-0">
		...
		<h3 class="page-title">Edit Alokasi Penugasan Pemeriksa</h3>
	</div>
</div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="publikasi_id" name="publikasi_id" required>
                                        ...
                                        @foreach($publikasi as $item)
                                        	<option value="{{ $item->id }}" {{ $assignment->publikasi_id == $item->id ? 'selected' : '' }}>
	                                    		{{ $item->nama_publikasi }}
	                                    	</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="pemeriksa_nip" name="pemeriksa_nip" required>
                                        ...
                                        @foreach($users as $item)
                                        	<option value="{{ $item->nip }}" {{ $assignment->pemeriksa_nip == $item->nip ? 'selected' : '' }}>
	                                    		{{ $item->fullname }}
	                                    	</option>
                                        @endforeach
                                    </select>
