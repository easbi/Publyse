    ...
    <div x-data="{ showModal: false }" class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <button @click="showModal = true" type="button"
                    ...
                    <div
                        class="absolute inset-0 bg-gradient-to-r from-blue-400 to-blue-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    </div>
        ...
        @if (isset($reviewerStats) && $reviewerStats->isNotEmpty())
            <div class="bg-white p-6 rounded-lg shadow mb-8">
                                ...
                                @if ($stat['total_comments'] > 0)
                                    <div class="mt-3">
                                        <div class="flex justify-between items-center mb-1">
                                            ...
                                            <span
                                                class="text-xs font-semibold text-gray-700">{{ $stat['completion_percentage'] }}%</span>
                                        </div>
                ...
                @php
                    ...
                    $overallCompletion =
                        $totalAllComments > 0 ? round(($totalDoneComments / $totalAllComments) * 100) : 0;
                @endphp
        ...
        <div class="bg-white p-6 rounded-lg shadow mb-8">
            ...
            <div class="w-full bg-gray-200 rounded-full h-4">
                <div class="bg-blue-600 h-4 rounded-full text-center text-white text-xs leading-4"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($allComments as $comment)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $comment->page_number ?: '-' }}
                                    </td>
                ...
                <form action="{{ route('superilis.generate', $publication) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex gap-4">
                            <label
                                class="flex items-center p-4 border rounded-lg has-[:checked]:bg-blue-50 has-[:checked]:border-blue-400 cursor-pointer">
                                <input type="radio" name="template_choice" value="A"
                                    class="form-radio h-4 w-4 text-blue-600" checked>
                                <span class="ml-3 text-sm">Template Word (Sudah TTD dan CAP)</span>
                            </label>
                            <label
                                class="flex items-center p-4 border rounded-lg has-[:checked]:bg-blue-50 has-[:checked]:border-blue-400 cursor-pointer">
                                <input type="radio" name="template_choice" value="B"
                    ...
                    <form action="{{ route('superilis.generate', $publication) }}" class="space-y-4 px-2">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            ...
                            <div class="flex-1">
                                <label for="tanggal_surat" class="block text-sm font-medium text-gray-700 mb-1">Tanggal
                                    Surat</label>
                                @php
                                    $maxTanggal = \Carbon\Carbon::parse($publication->release_date)
                                        ->subDays(3)
                                        ->format('Y-m-d');
                                @endphp
                                <input type="date" name="tanggal_surat" id="tanggal_surat"
                                    value="{{ old('tanggal_surat', $maxTanggal) }}" required
                                    max="{{ $maxTanggal }}" class="form-input w-full">
                            </div>
                        </div>

                        <div class="flex flex-col sm:flex-row gap-4">
                            <div class="flex-1">
                                <label for="nama_kepala" class="block text-sm font-medium text-gray-700 mb-1">Nama
                                    Kepala</label>
                                <input type="text" name="nama_kepala" id="nama_kepala"
                                    value="{{ old('nama_kepala', \App\Models\Superilis::where('key', 'last_head_name')->first()->value ?? '') }}"
                                    required class="form-input w-full">
                            </div>
                            <div class="flex-1">
                                <label for="nip_kepala" class="block text-sm font-medium text-gray-700 mb-1">NIP
                                    Kepala</label>
                                <input type="text" name="nip_kepala" id="nip_kepala"
                                    value="{{ old('nip_kepala', \App\Models\Superilis::where('key', 'last_head_nip')->first()->value ?? '') }}"
                                    required class="form-input w-full">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="tipe_buku" id="tipe_buku" class="form-select w-full">
                                ...
                                <option value="Non-ARC" {{ old('tipe_buku') == 'Non-ARC' ? 'selected' : '' }}>Non-ARC
                                </option>
                            </select>
                        </div>
        ...
        $(document).ready(function() {
            ...
            $('#commentsTable').DataTable({
                ...
                "order": [
                    [0, "desc"],
                    [1, "asc"]
                ], // Sort by version desc, then page asc
                "columnDefs": [{
                        "orderable": true,
                        "targets": [0, 1, 2, 4, 5]
                    }, // Version, Page, Reviewer, Status, Date sortable
                    {
                        "orderable": false,
                        "targets": [3]
                    }, // Comment content not sortable
                    {
                        "type": "num",
                        "targets": [0, 1]
                    } // Version and page as numbers
                ],
                "language": {
                    "search": "Cari:",
